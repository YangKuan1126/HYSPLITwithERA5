...
def plot_month(tracks_3d, labels, shp_path, outfile,
    ...
    # ----------① 改裁剪范围 ----------
    ax.set_extent([-60, 180, 0, 90], crs=ccrs.PlateCarree())
    ...
    for lab, tr in zip(labels, tracks_3d):
        ax.plot(tr[:,1], tr[:,0], color=colors[lab],
                lw=lw, alpha=alpha, transform=ccrs.PlateCarree())

    # ----------② 改经线 ----------
    xticks = [-60, -20, 20, 60, 100, 140, 180]
    yticks = [20, 40, 60, 80]
    ax.set_xticks(xticks, crs=ccrs.PlateCarree())
    ax.set_yticks(yticks, crs=ccrs.PlateCarree())
    ax.set_xticklabels([f'{x}°E' for x in xticks], fontsize=4)
