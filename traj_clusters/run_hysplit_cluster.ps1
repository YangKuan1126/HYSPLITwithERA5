...
# 月标签
$monTag = if ($Months.Count) {
    ($Months | %{'{0:d2}' -f $_}) -join ''
} else { 'all' }

# 主循环
foreach ($pt in $Points) {
    Write-Host "===== 处理点: $pt =====" -ForegroundColor Cyan
    $archiveDir = "$hysRoot\cluster\archive\${YearStart}_${YearEnd}_${monTag}_$pt"
    New-Item -ItemType Directory -Path $archiveDir -Force | Out-Null

    # 1. 生成 INFILE
    $infile = Join-Path $workDir 'INFILE'
    $pyArgs = @($pyVersion,$scriptIN,'--root',$TrajRoot,'--outfile',$infile,'--years',$YearStart,$YearEnd,'--ref-subdir',$pt)
    ...
    cd D:\Github\HYSPLITwithERA5\traj_clusters
    ...
    if ($meanFiles.Count -gt 0) {
        ...
        Write-Warning "[merglist] 未找到任何 mean 文件，跳过合并步骤"
    }
    ...
    Write-Host "完成绘图" -ForegroundColor Green
    # 12. 归档
...
# C:\hysplit\exec\trajplot.exe -a3 -A3 -f0 -g1000 +g1 -imerged_mean.tdump -jarlmap -k3 -l24 -L1 -m0 -o1979_1980_P1_TRAJMEAN -s1 -v4 -z67